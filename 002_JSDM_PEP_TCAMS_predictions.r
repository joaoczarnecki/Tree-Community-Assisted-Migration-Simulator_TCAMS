...
# --- Ensure covariates share identical order/structure across scenarios ---
reference_order <- colnames(XData)
factor_columns <- names(Filter(is.factor, as.data.frame(XData)))
XData_future_list <- lapply(XData_future_list, function(df) {
  # Align factor levels (e.g., drainage_class) to the fitted model
  for (fc in factor_columns) {
    if (fc %in% colnames(df)) {
      df[[fc]] <- factor(df[[fc]], levels = levels(XData[[fc]]))
    }
  }
  missing_cols <- setdiff(reference_order, colnames(df))
  if (length(missing_cols)) {
    stop("The following predictors are missing in XData_future_list entry: ",
         paste(missing_cols, collapse = ", "))
  }
  df[, reference_order, drop = FALSE]
})

# --- 4.2 Pre-calculate Predictions with Uncertainty ---
predictions_file <- file.path(shiny_app_dir, "all_scenario_predictions.Rda")
scenario_names <- names(XData_future_list)
...
save(plots_for_map, plot_coordinates, file = file.path(shiny_app_dir, "plot_coordinates.RData"))
...
# --- Quick diagnostics (remove after verification) ---
if (length(XData_future_list)) {
  ref_order <- colnames(XData_future_list[[1]])
  order_checks <- vapply(XData_future_list, function(df) identical(colnames(df), ref_order), logical(1))
  message("[Diag] XData column order aligned across scenarios: ", all(order_checks))
  if (!all(order_checks)) {
    warning("[Diag] Mismatched predictor order detected for: ", paste(names(order_checks)[!order_checks], collapse = ", "))
  }
}

if (exists("all_predictions_summary") && length(all_predictions_summary)) {
  diag_summary <- purrr::imap_dfr(all_predictions_summary, function(pred_obj, scenario) {
    tibble(
      scenario = scenario,
      ERR_mean = mean(pred_obj$mean$ERR_n, na.rm = TRUE),
      SAB_mean = mean(pred_obj$mean$SAB_n, na.rm = TRUE)
    )
  })
  message("[Diag] Posterior mean comparison (ERR_n & SAB_n):")
  print(diag_summary)
}

if (exists("TYPE_ECO_list") && length(TYPE_ECO_list) && exists("all_predictions_summary") && length(all_predictions_summary)) {
  available_species <- colnames(all_predictions_summary[[1]]$mean)
  type_subset <- utils::head(TYPE_ECO_list, 2)
  purrr::iwalk(type_subset, function(species_vec, nm) {
    missing_sp <- setdiff(species_vec, available_species)
    if (length(missing_sp)) {
      warning("[Diag] TYPE_ECO '", nm, "' missing species in predictions: ", paste(missing_sp, collapse = ", "))
    }
  })
}
...
save(plots_for_map, plot_coordinates, file = file.path(output_dir, "hmsc_fixed_effects", "plot_coordinates.RData"))
